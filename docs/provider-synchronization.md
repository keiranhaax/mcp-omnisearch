...
| Processing runtime response validation (`4ea9d2211bb4493a584433dde8c79aebde6c8848`) | applicable             | Ported for Exa Contents, Exa Similar, and Tavily Extract through current-provider schemas, and for retained Firecrawl paths through the shared validated Firecrawl envelope. Kagi summarization is removed.                                                                           |
...
| Provider coverage tests (`67b07702e0e9a3cf2699c14cfcc75d50dd89caf3`)                | already implemented    | Retained providers have local tests adapted to current APIs and fork behavior; this task adds malformed-response and relaxed-valid-shape coverage. Kagi coverage is intentionally absent.                                                                                             |
| Firecrawl provider coverage tests (`8d6f409330562d1c68575a82749d712bd7b62171`)      | already implemented    | Retained Firecrawl paths already have local coverage; this task adds current search-highlight/plain-description, document/PDF scrape, crawl-shape, and shared-envelope validation fixtures.                                                                                           |
...
- `4ea9d2211bb4493a584433dde8c79aebde6c8848`: Exa Contents, Exa
  ...
  Firecrawl Actions, Crawl, Extract, Map, and Scrape use the shared
  validated Firecrawl envelope; fork-only Firecrawl Search and Agent
  use that same shared path. Kagi summarization is removed. Fork-only
  Brave LLM Context is absent from the pinned diff.
...
metadata may be absent; and current Firecrawl responses may omit
`success`, use terminal `failed`/`cancelled` states, contain nullable
page content, and place page URLs/errors in metadata.
